            ...
            <form action="{{ route('orcamentos.index') }}" method="GET">
                <div class="row g-3 align-items-end">
                    <div class="col-md-6 col-lg-4">
                        ...
                        <input type="hidden" id="filtro_cliente_id" name="filtro_cliente_id" value="{{ request('filtro_cliente_id') }}">
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <label for="filtro_cliente_nome_avulso" class="form-label form-label-sm">Ou Cliente Avulso (Nome):</label>
                        <input type="text" class="form-control form-control-sm" id="filtro_cliente_nome_avulso"
                               name="filtro_cliente_nome_avulso"
                    ...
                    <div class="col-md-6 col-lg-3">
                        ...
                        <input type="date" class="form-control form-control-sm" id="data_emissao_de" name="data_emissao_de" value="{{ request('data_emissao_de') }}">
                    </div>
                    <div class="col-md-6 col-lg-3">
                        <label for="data_emissao_ate" class="form-label form-label-sm">Emissão Até:</label>
                        <input type="date" class="form-control form-control-sm" id="data_emissao_ate" name="data_emissao_ate" value="{{ request('data_emissao_ate') }}">
                    </div>
                    <div class="col-md-6 col-lg-3">
                        <label for="filtro_criado_por_id" class="form-label form-label-sm">Criado por:</label>
                        <select class="form-select form-select-sm" id="filtro_criado_por_id" name="filtro_criado_por_id">
                    ...
                    <div class="col-md-12 col-lg-3 d-flex align-items-end">
                        <button class="btn btn-primary btn-sm me-2 w-100 mb-1 mb-lg-0" type="submit"><i class="bi bi-search"></i> Filtrar</button>
                        <a href="{{ route('orcamentos.index') }}" class="btn btn-secondary btn-sm w-100"><i class="bi bi-eraser-fill"></i> Limpar</a>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($orcamento->cliente)
                                <a href="{{ route('clientes.show', $orcamento->cliente->id) }}">{{ Str::limit($orcamento->cliente->nome_completo, 25) }}</a>
                            @else
                                {{ Str::limit($orcamento->nome_cliente_avulso, 25) ?? 'N/A (Avulso)' }}
                                @if($orcamento->telefone_cliente_avulso) <small class="d-block text-muted">Tel: {{ $orcamento->telefone_cliente_avulso }}</small> @endif
                            @endif
                        </td>
                        <td>{{ Str::limit($orcamento->descricao_aparelho, 30) }}</td>
                        <td>{{ $orcamento->data_emissao->format('d/m/Y') }}</td>
                        <td>{{ $orcamento->data_validade ? $orcamento->data_validade->format('d/m/Y') : ($orcamento->validade_dias ? $orcamento->validade_dias . ' dias' : 'N/A') }}</td>
                        ...
                        <td class="text-center">
                            @php
                                ...
                                elseif ($orcamento->status === 'Reprovado') $statusClass = 'bg-danger';
                                elseif ($orcamento->status === 'Cancelado') $statusClass = 'bg-secondary';
                                elseif ($orcamento->status === 'Aguardando Aprovação') $statusClass = 'bg-warning text-dark';
                                elseif ($orcamento->status === 'Em Elaboração') $statusClass = 'bg-info text-dark';
                                elseif ($orcamento->status === 'Convertido em OS') $statusClass = 'bg-primary';
                        ...
                        <td class="text-center">
                            <a href="{{ route('orcamentos.show', $orcamento->id) }}" class="btn btn-info btn-sm" title="Ver Detalhes">
                                <i class="bi bi-eye-fill"></i>
                            </a>
                            @if(in_array($orcamento->status, ['Em Elaboração']))
                                <a href="{{ route('orcamentos.edit', $orcamento->id) }}" class="btn btn-warning btn-sm" title="Editar">
