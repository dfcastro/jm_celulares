...
<style>
    ...
    .badge.bg-status-diagnostico {
        background-color: #17a2b8 !important;
        color: white;
    }

    /* Bootstrap 'info' */
    .badge.bg-status-aguardando {
        background-color: #ffc107 !important;
        color: #212529;
    }

    /* Bootstrap 'warning' */
    .badge.bg-status-manutencao {
        background-color: #0d6efd !important;
        color: white;
    }

    /* Bootstrap 'primary' */
    .badge.bg-status-pronto {
            ...
            <form action="{{ route('atendimentos.index') }}" method="GET">
                <div class="row g-3">
                    ...
                    <div class="col-md-6 col-lg-3">
                        <label for="filtro_status" class="form-label">Status Serviço:</label>
                        <select class="form-select form-select-sm" id="filtro_status" name="filtro_status">
                            <option value="">Todos os Status</option>
                            @foreach ($todosOsStatus as $statusValue) {{-- $todosOsStatus no controller deve vir de Atendimento::getPossibleStatuses() --}}
                ...
                <div class="col-md-6 col-lg-3"> {{-- Ajuste o tamanho da coluna conforme necessário --}}
                    <label for="filtro_status_pagamento" class="form-label">Status Pagamento:</label>
        ...
        <table class="table table-striped table-hover table-bordered table-sm"> {{-- table-sm para mais compacta --}}
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <span class="badge rounded-pill status-badge {{ App\Models\Atendimento::getPaymentStatusClass($atendimento->status_pagamento) }}" title="Status do Pagamento: {{ $atendimento->status_pagamento ?? 'Pendente' }}">
                            <i class="bi {{ App\Models\Atendimento::getPaymentStatusIcon($atendimento->status_pagamento) }}"></i>
                            {{-- Opcional: exibir o texto do status de pagamento se houver espaço --}}
                ...
                @empty
                <tr>
                    <td colspan="8" class="text-center py-4">Nenhum atendimento encontrado com os filtros aplicados.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
