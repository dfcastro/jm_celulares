...
<style>
    ...
    .bg-light-subtle { background-color: #f8f9fa !important; } /* Um cinza mais claro para consistência */
    ...
    @media (max-width: 767.98px) { /* Abaixo de tablets (Bootstrap md breakpoint) */
    #statusAtualTexto.badge { /* Usando o ID para ser específico */
        font-size: 0.8rem !important; /* Reduz o tamanho da fonte do badge */
        padding: .3em .6em !important; /* Reduz o padding interno do badge */
        white-space: normal; /* Permite que o texto dentro do badge quebre linha se necessário */
        text-align: left; /* Alinha o texto quebrado à esquerda dentro do badge */
        display: inline-block; /* Garante que o badge se comporte bem com quebra de linha interna */
        max-width: 100%; /* Garante que o badge não ultrapasse seu contêiner */
    }
    #statusAtualTexto.badge .bi {
        font-size: 0.9em; /* Reduz um pouco o ícone se necessário */
    }
}

@media (max-width: 575.98px) { /* Telas muito pequenas (Bootstrap sm breakpoint) */
    #statusAtualTexto.badge {
        font-size: 0.75rem !important; /* Ainda menor para telas extra pequenas */
...
<div class="container mt-0">
    <div class="actions-header mb-3">
        ...
        <div>
            ...
            <a href="{{ route('atendimentos.edit', $atendimento->id) }}" class="btn btn-sm btn-outline-warning me-1" title="Edição Completa">
                        ...
                        <div class="col-md-6">
                            ...
                            <dl class="dl-horizontal">
                                ...
                                <dd>
                                    <span id="statusPagamentoTextoOuter">
                                        @include('atendimentos.partials._status_pagamento_badge', ['status_pagamento' => $atendimento->status_pagamento])
                                    </span>
                                </dd>
                                <dt>Técnico Resp.:</dt>
                                <dd id="tecnicoAtualTexto">{{ $atendimento->tecnico->name ?? 'Não atribuído' }}</dd>
                                <dt>Data Conclusão:</dt>
                                <dd>{{ $atendimento->data_conclusao ? $atendimento->data_conclusao->format('d/m/Y') : ($atendimento->isFinalizadoParaLista() ? 'Concluído/Finalizado' : 'Pendente') }}</dd>
                            </dl>
                            ...
                            <div class="mt-2 mb-3 p-2 border rounded bg-light-subtle">
                                <form action="{{ route('atendimentos.atualizarStatus', $atendimento->id) }}" method="POST" class="d-flex align-items-center flex-wrap" id="formAtualizarStatusRapido">
                                    @csrf @method('PATCH')
                                    <label for="status_rapido" class="form-label me-2 mb-0 fw-semibold small">Alterar Status Serviço:</label>
                                    <select class="form-select form-select-sm flex-grow-1 me-2 mb-1 mb-sm-0" style="min-width: 170px;" id="status_rapido" name="status">
                            ...
                            <div id="containerBotaoRegistrarPagamento" class="mt-3" style="display: none;">
            ...
            <div class="card mb-4 shadow-sm">
                ...
                <div class="card-body">
                    <h6 class="section-title">
                        ...
                        <button type="button" class="btn btn-link btn-sm p-0 ms-1" id="btnEditarLaudo" title="Editar Laudo"><i class="bi bi-pencil-square"></i></button>
                        @endcan
                    </h6>
                    <div id="containerLaudo">
                        <div id="textoLaudo" class="laudo-box">{{ $atendimento->laudo_tecnico ?: 'Ainda não informado.' }}</div>
                        @can('is-admin-or-tecnico')
                    ...
                    <h6 class="section-title mt-3">
                        ...
                        @can('is-admin-or-tecnico')
                        <button type="button" class="btn btn-link btn-sm p-0 ms-1" id="btnEditarObservacoes" title="Editar Observações"><i class="bi bi-pencil-square"></i></button>
                        @endcan
                    </h6>
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-4 shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @can('is-admin-or-tecnico')
                    <a href="{{ route('saidas-estoque.create', ['atendimento_id' => $atendimento->id]) }}" class="btn btn-success btn-sm py-0 px-1" title="Adicionar Peça">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-striped table-hover mb-0">
                            <thead class="table-light visually-hidden"> {{-- Cabeçalho pode ser oculto para economizar espaço --}}
                                <tr><th>Peça</th><th class="text-center">Qtd.</th><th class="text-end">Subtotal</th></tr>
                            </thead>
                            <tbody>
            ...
            <div class="card mb-4 shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @can('is-admin')
                    <button type="button" class="btn btn-outline-primary btn-sm py-0 px-1" id="btnEditarValoresServico" title="Editar Valores">
                ...
                <div class="card-body">
                    @php
                        ...
                        if ($atendimento->saidasEstoque) {
                            foreach ($atendimento->saidasEstoque as $saida) {
                                if ($saida->estoque) $valorTotalPecas += $saida->quantidade * ($saida->estoque->preco_venda ?? 0);
                            }
                        }
                    ...
                    <div id="areaExibirValores">
                        <dl class="dl-horizontal mb-0">
                            ...
                            <dd class="text-end" id="textoValorServico"><small>R$ {{ number_format($valorServicoAtual, 2, ',', '.') }}</small></dd>
                            <dt><small>Desconto Serv.:</small></dt>
                            <dd class="text-end text-danger" id="textoDescontoServico"><small>- R$ {{ number_format($descontoServicoAtual, 2, ',', '.') }}</small></dd>
                            <dt class="fw-semibold"><small>Subtotal Serviço:</small></dt>
                            <dd class="text-end fw-bold" id="textoSubtotalServico"><small>R$ {{ number_format($valorServicoLiquido, 2, ',', '.') }}</small></dd>
                            <dt class="mt-2"><small>Total Peças:</small></dt>
                            <dd class="text-end mt-2" id="textoValorTotalPecas"><small>R$ {{ number_format($valorTotalPecas, 2, ',', '.') }}</small></dd>
                            <dt class="border-top pt-2 fs-6 text-success"><small>TOTAL OS:</small></dt>
                            <dd class="text-end border-top pt-2 fs-5 fw-bolder text-success" id="textoValorTotalAtendimento">R$ {{ number_format($valorTotalAtendimento, 2, ',', '.') }}</dd>
                        </dl>
                    </div>
                    @can('is-admin')
                    <div id="formEditarValoresServico" style="display:none;" class="mt-2 pt-2 border-top">
                        <div class="mb-2">
                            <label for="inputValorServico" class="form-label form-label-sm fw-semibold">Valor Serviço (R$):</label>
                            <input type="number" step="0.01" class="form-control form-control-sm" id="inputValorServico" value="{{ number_format($valorServicoAtual, 2, '.', '') }}">
                        </div>
                        <div class="mb-2">
                            <label for="inputDescontoServico" class="form-label form-label-sm fw-semibold">Desconto (R$):</label>
                            <input type="number" step="0.01" class="form-control form-control-sm" id="inputDescontoServico" value="{{ number_format($descontoServicoAtual, 2, '.', '') }}">
                        </div>
                        <button type="button" class="btn btn-sm btn-success me-1" id="btnSalvarValoresServico"><i class="bi bi-check-lg"></i> Salvar</button>
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-info">
                        ...
                        <p class="mb-0"><strong>Valor Total Original do Atendimento:</strong>
                            <strong id="valorTotalOriginalDisplayModal">R$ {{ number_format($valorTotalDevidoModal ?? 0, 2, ',', '.') }}</strong>
                        </p>
                    </div>
                    <hr>
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control"
                               id="modal_valor_servico" name="valor_servico"
                               value="{{ number_format($valorServicoModal ?? 0, 2, '.', '') }}"
                               min="0" {{ Gate::denies('is-admin') ? 'readonly' : '' }}>
                        @cannot('is-admin') <small class="form-text text-muted">Apenas administradores podem alterar este valor.</small> @endcannot
                    </div>
                    <div class="mb-3">
                        <label for="modal_desconto_servico" class="form-label">Desconto sobre Serviço R$ (Opcional):</label>
                        <input type="number" step="0.01" class="form-control"
                               id="modal_desconto_servico" name="desconto_servico"
                               value="{{ number_format($descontoServicoModal ?? 0, 2, '.', '') }}"
                               min="0" {{ Gate::denies('is-admin') ? 'readonly' : '' }}>
                     ...
                     <div class="mb-3 p-2 bg-light border rounded">
                        <p class="mb-1">Subtotal do Serviço (após desconto):
                            <strong id="modalSubtotalServicoDisplay">R$ {{ number_format($valorServicoLiquidoModal ?? 0, 2, ',', '.') }}</strong>
                        </p>
                        <p class="mb-1">(+) Valor Total das Peças:
                            <strong id="modalValorPecasDisplay">R$ {{ number_format($valorTotalPecasModal ?? 0, 2, ',', '.') }}</strong>
                        </p>
                        <p class="mb-0 fs-5">(=) VALOR A PAGAR:
                            <strong id="modalNovoValorTotalDevidoDisplay" class="text-primary">R$ {{ number_format($valorTotalDevidoModal ?? 0, 2, ',', '.') }}</strong>
                        </p>
                    </div>
                    <hr>
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="modal_forma_pagamento" name="forma_pagamento" required>
                            ...
                            @if(isset($formasPagamentoDisponiveis) && (is_array($formasPagamentoDisponiveis) || is_object($formasPagamentoDisponiveis)))
                                ...
                                @endforeach
                            @else
                                <option value="Dinheiro">Dinheiro</option><option value="Cartão de Débito">Cartão de Débito</option><option value="Cartão de Crédito">Cartão de Crédito</option><option value="PIX">PIX</option><option value="Boleto">Boleto</option><option value="Outro">Outro</option>
                            @endif
                        </select>
                    </div>
                    <div class="mb-3">
...
@push('scripts')
    ...
    {{-- Certifique-se de copiar todo o conteúdo entre <script> e </script> da resposta anterior --}}
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    let statusPagamentoAtualJs = "{{ $atendimento->status_pagamento }}";
    ...
    function calcularTotaisModal() {
        ...
        if (!modalValorServicoInput || !modalDescontoServicoInput || !modalSubtotalServicoDisplay || !modalNovoValorTotalDevidoDisplay || !modalValorPecasDisplay) {
            console.error("Um ou mais elementos do modal para cálculo de totais não foram encontrados.");
            return;
        }

        let valorServico = parseFloat(modalValorServicoInput.value.replace(',', '.')) || 0; // Tenta substituir vírgula por ponto para o parseFloat
        let descontoServico = parseFloat(modalDescontoServicoInput.value.replace(',', '.')) || 0;

        if (descontoServico > valorServico) {
            descontoServico = valorServico;
            modalDescontoServicoInput.value = descontoServico.toFixed(2).replace('.', ','); // Formata para exibição com vírgula
        }

        let subtotalServico = valorServico - descontoServico;
        // valorTotalPecasFixoModal já é um número
        let novoValorTotalDevido = subtotalServico + valorTotalPecasFixoModal;

        // Atualizando os displays. Usar toLocaleString para formatação de moeda brasileira.
    ...
    if (modalValorServicoInput && modalDescontoServicoInput) {
        modalValorServicoInput.addEventListener('input', calcularTotaisModal);
        modalDescontoServicoInput.addEventListener('input', calcularTotaisModal);
    }

    // Quando o modal é aberto, preenchemos os valores e calculamos
    if (modalRegistrarPagamento) { // modalRegistrarPagamento é o elemento do modal em si
        modalRegistrarPagamento.addEventListener('show.bs.modal', function () {
            ...
            if(modalValorServicoInput) modalValorServicoInput.value = valorServicoAtualPg.toFixed(2); // Usar toFixed(2) para manter o formato .00
            ...
            calcularTotaisModal(); // Calcula e atualiza os displays no modal ao abrir
        });
    }
    ...
    function atualizarInputsModalPagamento(novosValoresNumericos) {
        if (modalValorServicoInput && typeof novosValoresNumericos.valor_servico_raw !== 'undefined') {
            modalValorServicoInput.value = parseFloat(novosValoresNumericos.valor_servico_raw).toFixed(2);
        }
        if (modalDescontoServicoInput && typeof novosValoresNumericos.desconto_servico_raw !== 'undefined') {
            modalDescontoServicoInput.value = parseFloat(novosValoresNumericos.desconto_servico_raw).toFixed(2);
        }
        calcularTotaisModal();
    }
    ...
    if (btnEditarValoresServicoPage && areaExibirValoresPage && formEditarValoresPage) {
        ...
        btnEditarValoresServicoPage.addEventListener('click', function() {
            ...
            inputValorServicoPage.value = (parseFloat(textoValorServicoPage.innerText.replace('R$ ', '').replace(/\./g, '').replace(',', '.')) || 0).toFixed(2);
            inputDescontoServicoPage.value = (parseFloat(textoDescontoServicoPage.innerText.replace('- R$ ', '').replace(/\./g, '').replace(',', '.')) || 0).toFixed(2);
            ...
            btnSalvarValoresPage.addEventListener('click', function() {
                ...
                .then(({statusHttp, body}) => {
                    if (body.success && body.novos_valores) {
                        textoValorServicoPage.innerText = 'R$ ' + body.novos_valores.valor_servico;
                        textoDescontoServicoPage.innerText = '- R$ ' + body.novos_valores.desconto_servico;
                        textoSubtotalServicoPage.innerText = 'R$ ' + body.novos_valores.subtotal_servico;
                        textoValorTotalAtendimentoPage.innerText = 'R$ ' + body.novos_valores.valor_total_atendimento;
                        exibirFeedbackGlobal(body.message, 'success');
            ...
            .then(({statusHttp, body}) => {
                if (body.success) {
                    ...
                    if (body.novos_valores_atendimento) {
                        textoValorServicoPage.innerText = 'R$ ' + body.novos_valores_atendimento.valor_servico;
                        textoDescontoServicoPage.innerText = '- R$ ' + body.novos_valores_atendimento.desconto_servico;
                        textoSubtotalServicoPage.innerText = 'R$ ' + body.novos_valores_atendimento.subtotal_servico;
                        ...
                        textoValorTotalAtendimentoPage.innerText = 'R$ ' + body.novos_valores_atendimento.valor_total_atendimento;
                    }
