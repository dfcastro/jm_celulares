                        ...
                        <div class="mt-2 mb-3 p-2 border rounded bg-light-subtle">
                            <form action="{{ route('atendimentos.atualizarStatus', $atendimento->id) }}" method="POST" class="d-flex align-items-center flex-wrap" id="formAtualizarStatusRapido">
                                ...
                                <select class="form-select form-select-sm flex-grow-1 me-2 mb-1 mb-sm-0" style="min-width: 180px;" id="status_rapido" name="status" aria-label="Alterar status do atendimento">
                                    @php
                                        // Supondo que você tenha um método estático no Model Atendimento
                                        // ou uma lista definida no controller para $todosOsStatus.
                                        // Ex: $todosOsStatus = \App\Models\Atendimento::getPossibleStatuses();
                                        $todosOsStatus = ['Em diagnóstico', 'Aguardando peça', 'Em manutenção', 'Pronto para entrega', 'Entregue', 'Cancelado', 'Reprovado']; // Defina seus status
                                    @endphp
...
@push('styles')
    <style>
        .badge.fs-6 { font-size: 0.95rem !important; padding: .5em .9em; }
        .bg-light-subtle { background-color: #fcfcfc !important; border-color: #e9ecef !important; }
        .user-select-all { user-select: all; }
        #feedbackGlobalAtendimentoShow .alert { margin-bottom: 1rem; padding: 0.75rem 1.25rem; font-size: 0.9rem;}
    </style>
@endpush
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupInlineEdit(config) {
        ...
        if (btnEdit && textDisplay && formEdit && inputElement && btnSave && btnCancel && feedbackElement) {
            ...
            btnSave.addEventListener('click', function() {
                ...
                .then(response => {
                    if (!response.ok) {
                         return response.json().then(errData => Promise.reject({ status: response.status, data: errData, responseText: JSON.stringify(errData) }));
                    }
    ...
    @can('is-admin-or-tecnico')
    setupInlineEdit({
        btnEditId: 'btnEditarObservacoes', textDisplayId: 'textoObservacoes', formEditId: 'formEditarObservacoes', inputId: 'inputObservacoes',
        btnSaveId: 'btnSalvarObservacoes', btnCancelId: 'btnCancelarObservacoes', feedbackId: 'feedbackObservacoes',
    ...
    setupInlineEdit({
        btnEditId: 'btnEditarLaudo', textDisplayId: 'textoLaudo', formEditId: 'formEditarLaudo', inputId: 'inputLaudo',
        btnSaveId: 'btnSalvarLaudo', btnCancelId: 'btnCancelarLaudo', feedbackId: 'feedbackLaudo',
    ...
    const feedbackStatusRapido = document.getElementById('feedbackStatusRapido'); // Feedback local para o formulário de status
    ...
    if (formAtualizarStatusRapido) {
        formAtualizarStatusRapido.addEventListener('submit', function(event) {
            ...
            if (feedbackStatusRapido) feedbackStatusRapido.innerHTML = '<span class="text-info small">Atualizando status...</span>';

            fetch(url, {
    method: 'POST', // FormData será enviado como POST, Laravel usa _method
    headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        'Accept': 'application/json', // Garante que $request->wantsJson() seja true
        'X-Requested-With': 'XMLHttpRequest' // Garante que $request->ajax() seja true
    },
    body: formData
})
.then(response => {
    if (!response.ok) {
        // Tenta pegar o corpo da resposta como JSON, mesmo para erros
        return response.json().then(errData => {
            // Adiciona o status da resposta ao objeto de erro para melhor depuração
            return Promise.reject({ status: response.status, data: errData, responseText: JSON.stringify(errData) });
        }).catch(jsonParseError => {
            // Se não conseguir parsear como JSON (ex: é HTML puro de um erro 500 não JSON)
            // Retorna o texto da resposta para que o catch principal possa logar
            return response.text().then(textData => Promise.reject({ status: response.status, data: null, responseText: textData }));
        });
    }
    return response.json();
})
.then(data => {
    ...
    if (data.success) {
        ...
        if (statusAtualTextoSpan && statusAtualNomeSpan && data.novo_status && data.novo_status_classe_completa && data.novo_status_icon) {
            ...
            // statusAtualTextoSpan é o span externo do badge, precisa ter o ID 'statusAtualTexto'
            statusAtualTextoSpan.className = 'badge rounded-pill fs-6 ' + data.novo_status_classe_completa;
            // Atualizar o ícone dentro do badge
        ...
        exibirFeedbackGlobal(data.message || 'Erro ao atualizar status.', 'danger');
    }
    // ...
})
.catch(error => {
    ...
    if (error && error.status === 422 && error.data && error.data.errors) { // Erros de validação do Laravel
    ...
    } else if (error && error.data && error.data.message) { // Erro JSON estruturado do seu backend
        errorMsg = error.data.message;
    } else if (error && error.responseText) { // Resposta não JSON (HTML de erro)
        console.error("Resposta não JSON recebida:", error.responseText); // Loga o HTML
        errorMsg = "O servidor respondeu com um erro inesperado. Verifique o console para detalhes (aba Network > Response).";
    } else if (error && error.message) { // Erro de rede ou outro erro JavaScript
            ...
            navigator.clipboard.writeText(textoParaCopiar)
                .then(() => {
                    if (mensagemCopiado) {
                        ...
                        setTimeout(() => { mensagemCopiado.style.display = 'none'; }, 2000);
                    }
                    if (btnCopiarCodigo) {
...
if (btnEditarValores && areaExibirValores && formEditarValores) {
    ...
    btnSalvarValores.addEventListener('click', function() {
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(errData => Promise.reject({ status: response.status, data: errData }));
            }
            return response.json();
        })
        .then(data => {
